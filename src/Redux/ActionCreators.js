...
export const FetchDataWeather = ()=>dispatch=>{
  ...
  //    }
  //
  // }) // bỏ kiểm tra location và lấy location
  let url = `https://api.openweathermap.org/data/2.5/forecast/daily?q=Danang&units=metric&cnt=14&appid=c0c4a4b4047b97ebc5948ac9c48c0559`;
  return api.weather.fetchdata(url)
  .then((data)=>{
    let datacurrent = {name:data.city.name,main:data.list[0].weather[0].main,time:api.weather.newDate(),description:data.list[0].weather[0].description,icon:data.list[0].weather[0].icon,temp:data.list[0].temp.day,rain:data.list[0].rain,humidity:data.list[0].humidity,speed:data.list[0].speed};
